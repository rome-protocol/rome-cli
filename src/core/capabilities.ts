...
export const CAPABILITIES: Capability[] = [
  ...
  verbAction(
    "bridge",
    "bridge",
    "Bridge in (--from <src>) or out (--to <dest>); --asset usdc (default, CCTP, any catalog chain) or eth (Wormhole: in → wETH on Rome; out → Ethereum only). Outbound you claim on the destination. Needs ROME_EVM_KEY. e.g. rome bridge hadrian --from sepolia --amount 0.002 --asset eth",
    [
      chainArg,
      { name: "from", required: false, description: "IN: source chain (id/name/slug) holding the asset" },
      { name: "to", required: false, description: "OUT: destination chain — you claim there (eth: Ethereum only)" },
      { name: "amount", required: true, description: "amount to bridge (human, e.g. 1.5 USDC or 0.002 ETH)" },
      { name: "asset", required: false, description: "usdc (default, CCTP) | eth (Wormhole)" },
      { name: "intent", required: false, description: "USDC IN only: gas (default) → native gas · wrapper → wUSDC on Rome" },
